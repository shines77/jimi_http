...
template <typename T, std::size_t CacheLineSize = CACHE_LINE_SIZE>
struct alignas(CacheLineSize)
atomic_padding : public std::atomic<typename std::remove_volatile<T>::type>,
                 public base_padding_data
{
    ...
    atomic_padding(_T val) : std::atomic<typename std::remove_volatile<T>::type>(val) {};
    ~atomic_padding() {};
};

template <typename T, std::size_t CacheLineSize = CACHE_LINE_SIZE>
struct atomic_padding_wrapper : public base_padding_data
{
    typedef typename std::remove_volatile<T>::type _T;
    typedef std::atomic<_T> value_type;
    ...
    atomic_padding_wrapper(_T value) : data(value) {};
